...
2. 전송할 맥락 패킷을 만든다.
   ...
   - 긴 코드, 로그, 스펙, diff는 필요한 부분만 압축 보고서로 만들고, 가능한 경우 파일로 업로드한다.
   - 판단에 필요한 파일이 여러 개라면 압축 파일 업로드를 기본값으로 삼는다. 불필요한 생성물, 중복 로그, vendor/dependency, 무관한 테스트 fixture는 tree-shaking한다.
   - 압축 파일에는 `BRIEF.md`, `MANIFEST.md`, 판단에 필요한 핵심 파일만 넣는다. 가능하면 핵심 파일은 10개 이하로 줄인다. 10개를 넘겨야 한다면 먼저 요약 보고서를 만들고, 외부 자문자가 추가 파일을 요구할 때 확장한다.
   - 스킬을 자문자에게 보내는 경우에는 예외적으로 tree-shaking하지 말고 해당 스킬 폴더 전체를 압축해 업로드한다. `SKILL.md`만 보내거나, `SKILL.md`가 직접 참조한 파일만 골라 보내지 않는다.
   - 스킬 폴더 압축에는 `SKILL.md`, `references/`, `scripts/`, `examples/`, `templates/`, 기타 하위 파일을 그대로 포함한다. 단, 명백한 비밀정보, 개인 인증 파일, 대용량 생성물, dependency 디렉터리가 섞여 있으면 제외하고 `MANIFEST.md`에 제외 사유를 적는다.
   - 파일 업로드가 도구나 브라우저 정책 때문에 불가능하면 하나의 Markdown 번들로 대체한다. 이 경우 최종 보고에서 "압축 업로드가 아니라 Markdown 번들/붙여넣기였다"고 명시한다.
