---
name: ask-gpt-pro
description: Use ChatGPT Pro on chatgpt.com as an external deep-thinking advisor or oracle for complex, ambiguous, high-stakes, or hard-to-solve problems. Trigger when the user asks to consult GPT Pro, ChatGPT Pro, an external advisor, a second brain, an oracle, or wants very deep reasoning beyond the current agent's local analysis, especially when browser access, file uploads, compact context packets, iterative follow-up questions, or answer-quality ping-pong are needed.
---
...
ChatGPT Pro를 독립적인 자문자로 사용한다. 현재 Codex가 가진 판단을 그대로 떠넘기지 말고, GPT Pro가 스스로 판단할 수 있도록 문제 맥락, 증거, 실패 이력, 제약, 우려를 압축해서 제공한다.

사용자가 "GPT Pro에게 물어봐", "oracle처럼 자문 받아봐", "이 문제 깊게 판단받아봐"라고 요청하면, 분석만 하지 말고 브라우저로 chatgpt.com에 접속해 새 대화를 만들고 답변 품질이 충분할 때까지 후속 질문을 이어간다.
...
1. 문제를 독립적으로 정리한다.
   ...
   - GPT Pro가 판단하지 않아도 되는 잡음, 중복 로그, 장황한 히스토리는 줄인다.
...
4. chatgpt.com에서 새 대화를 시작한다.
   - chatgpt.com으로 이동한다.
   - ChatGPT Pro 또는 사용 가능한 최상위 Pro/Reasoning 모델을 선택한다. 모델명이 바뀌었으면 화면에 표시된 최상위 심층 추론 모델을 사용하고, 선택한 모델명을 결과에 기록한다.
...
- GPT Pro의 답변을 최종 진실로 취급하지 말고, 외부 자문 결과로 취급한다.
- 사용자에게는 "GPT Pro가 이렇게 말했다"보다 "이 결론이 유효해 보이는 이유 / 적용할 액션 / 남은 리스크" 중심으로 보고한다.
- 코드 변경이 필요한 결론이면, 사용자가 명시적으로 review-only를 요청하지 않은 한 직접 적용하고 검증한다.
- GPT Pro가 현재 repo나 실제 파일을 보지 못한 상태에서 추정한 내용은 추정이라고 표시한다.
